...
{
    public class GetByIdQueueService : IGetByIdQueueService
    {

        private static readonly Gauge GetInProgress = Metrics.CreateGauge("get_by_id_queue_in_progress", "Get by id execution in progress");
        private static readonly Counter GetCounter = Metrics.CreateCounter("get_by_id_queue_counter", "Count the get by id execution");
        private static readonly Histogram GetDuration = Metrics.CreateHistogram("get_by_id_queue_duration", "Measure the duration of the get by id execution process");

        private readonly IGetPersonService _getPersonService;
        private readonly ICacheRepository _cacheRepository;

        public GetByIdQueueService(IGetPersonService? getPersonService,
        ...
        public void StartListening()
        {
            ...
            {
                channel.QueueDeclare(queue: "get-by-id",
                                     durable: true,
                                     exclusive: false,
                                     autoDelete: false,
                                     arguments: null);
                ...
                consumer.Received += async (model, ea) =>
                {
                    ...
                    using (GetDuration.NewTimer())
                    {
                        using (GetInProgress.TrackInProgress())
                        {
                            ...
                            GetPersonResponse transactionObject = Newtonsoft.Json.JsonConvert.DeserializeObject<GetPersonResponse>(message)!;
                            ...
                            try
                            {
                                personCreated = await this._getPersonService.Get(transactionObject.IdPerson);
                            }
                ...
                channel.BasicConsume(queue: "get-by-id",
                                     autoAck: true,
                                     consumer: consumer);
